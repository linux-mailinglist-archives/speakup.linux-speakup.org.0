Return-Path: <speakup+bounces-583-lists+speakup=lfdr.de@linux-speakup.org>
X-Original-To: lists+speakup@lfdr.de
Delivered-To: lists+speakup@lfdr.de
Received: from befuddled.reisers.ca (befuddled.reisers.ca [206.248.184.127])
	by mail.lfdr.de (Postfix) with ESMTP id 0925F597175
	for <lists+speakup@lfdr.de>; Wed, 17 Aug 2022 16:41:54 +0200 (CEST)
Received: by befuddled.reisers.ca (Postfix, from userid 65534)
	id 20A52384852; Wed, 17 Aug 2022 10:41:50 -0400 (EDT)
Received: from befuddled.reisers.ca (localhost [127.0.0.1])
	by befuddled.reisers.ca (Postfix) with ESMTP id 7F9EE3848B7
	for <lists+speakup@lfdr.de>; Wed, 17 Aug 2022 10:41:47 -0400 (EDT)
X-Original-To: speakup@linux-speakup.org
Delivered-To: speakup@linux-speakup.org
Received: by befuddled.reisers.ca (Postfix, from userid 65534)
	id 8FCB1384802; Wed, 17 Aug 2022 10:41:45 -0400 (EDT)
Received: from server2.shellworld.net (server2.shellworld.net [66.172.12.120])
	by befuddled.reisers.ca (Postfix) with ESMTPS id 5CDAE3847E6
	for <speakup@linux-speakup.org>; Wed, 17 Aug 2022 10:41:45 -0400 (EDT)
Received: by server2.shellworld.net (Postfix, from userid 1034)
	id 287AE121B89; Wed, 17 Aug 2022 14:41:41 +0000 (UTC)
Received: from localhost (localhost [127.0.0.1])
	by server2.shellworld.net (Postfix) with ESMTP id 25965121B7B;
	Wed, 17 Aug 2022 07:41:41 -0700 (PDT)
Date: Wed, 17 Aug 2022 07:41:40 -0700 (PDT)
From: Butch Bussen <butchb@shellworld.net>
To: "John G. Heim" <jheim@wisc.edu>
cc: K0LNY_Glenn <glenn@ervin.email>, Karen Lewellen <klewellen@shellworld.net>, 
...
Message-ID: <Pine.LNX.4.64.2208170740540.1352313@server2.shellworld.net>
...
MIME-Version: 1.0
Content-Type: MULTIPART/MIXED; BOUNDARY="-1404930036-1965092673-1660747300=:1352313"
X-Bogosity: Ham, tests=bogofilter, spamicity=0.000000, version=1.2.5

  This message is in MIME format.  The first part should be readable text,
  while the remaining parts are likely unreadable without MIME-aware tools.

---1404930036-1965092673-1660747300=:1352313
Content-Type: TEXT/PLAIN; charset=UTF-8; format=flowed
Content-Transfer-Encoding: QUOTED-PRINTABLE

Yes, I remember that.  And what about their oposition to descriptive=20
video, even filed a law sute to get the law thrown out, which it was at=20
that time.
73
Butch
WA0VJR
...
> Holy cow! Are you aware that the NFB once asked Microsoft to *NOT* improv=
e=20
> Narrator to the point where it would compete with Jaws? If you think the =
NFB=20
> is incapable of forcing choices on people, you are very sadly mistaken.
>
>
>
...
>>  Cc: "K0LNY_Glenn" <glenn@ervin.email>; <speakup@linux-speakup.org>; "Mi=
lan
...
>> >  In fact, provide if you do not mind an example of how  this works=20
>> >  exactly.
>> >  after all, unless I am incorrect, these employers are not personal Ja=
ws
>> >  users, meaning someone they trust continues to sell them on an expens=
ive
...
>> > >    If some of the clients in a call center already use Jaws, but don=
't=20
>> > >    know
...
>> > >    purchases,
>> > >    where a lot of Jaws purchases come from, it is in the agency's be=
st
>> > >    interest
>> > >    to spend as little as possible, because that doesn't come back li=
ke=20
>> > >    VR
>> > >    expenditures do.
>> > >    Glenn
...
>> > >=20
>> > >=20
>> > >    The challenge with that example is that, as one often gets remind=
ed,
>> > >  the
...
>> > >    we
>> > >  are
>> > >    creating an employment program where our clients will train in=20
>> > >    Linux,
>> > >    needing a solid screen reader solution for the system.  We will g=
ive=20
>> > >    you
>> > >    an
...
>> > > >    work,
>> > > >    and
>> > > >    probably at home, but they need to cater to the needs of the=20
>> > > >    client.
>> > > >    If a client used Linux, I doubt that any rehab counselor would
>> > > >  advocate
>> > > >    that
>> > > >    the client switch to Windows, unless that was needed for a=20
>> > > >    specific
>> > > >  job.
...
>> > > >    blinux
>> > > >    list
>> > > >    about how long it took completing tasks in the gui as apposed t=
o=20
>> > > >    say
>> > > >    command line,  the comments were quite informative.
>> > > >    Still, fs has never marketed largely to the end user. Instead t=
hey
>> > > >    market
>> > > >    to the American rehab community.
>> > > >    how much market research has  the rehab community done to suppo=
rt=20
>> > > >    the
>> > > >    need
>> > > >    for choices?
>> > > >    How many rehab counselors support  training in Linux?
>> > > >    one comment made by the subject of this thread about poor quali=
ty
>> > > >  speech
>> > > >    is a fine one...out of the box Linux has few speech choices.=20
>> > > >    everyone
...
>> > > >    On Sat, 13 Aug 2022, K0LNY_Glenn wrote:
>> > > >=20
>> > > > >    Well since Orca seems to work on so many distros, I don't kno=
w=20
>> > > > >  why
>> > > > >  FS
>> > > > >    would
>> > > > >    not be able to do the same.
>> > > > >    If Jaws users could switch into Linux, it would be a real gam=
e
>> > > > >    changer,
>> > > > >    and
>> > > > >    I think with lots more Blind Linux users, we would start seei=
ng
...
>> > > > >    for
>> > > > >    Linux.
>> > > > >    That is because as I understand it, Linux is  quite like clay=
=2E=20
>> > > > >    You
>> > > > >  can
...
>> > > > >    To build solid assistive tools one must have a solid  foundat=
ion=20
>> > > > >    as
>> > > > >  it
>> > > > >    were.  that is part of why there have needed to be so few App=
le
>> > > > >    efforts
>> > > > >    at
>> > > > >    inclusion, they  created  with, and then created in-house=20
>> > > > >    adaptive
>> > > > >    tools
...
>> > > > >    the
>> > > > >    former
>> > > > >    gw  micro or nvda to create something that can in theory  wor=
k.
...
>> > > > >    On Sat, 13 Aug 2022, K0LNY_Glenn wrote:
>> > > > >=20
>> > > > > >    I would like to see Freedom Scientific make a Jaws For Linu=
x.
>> > > > > >    JFL
>> > > > > >    I'd certainly pay the yearly rental fee for it, and it woul=
d=20
>> > > > > >    bring
>> > > > > >    many
>> > > > > >    more
...
>> > > > > >=20
>> > > > > >=20
>> > > > > > > > > > >    "KL" =3D=3D Karen Lewellen <klewellen@shellworld.=
net>
>> > > > > > > > > > >  writes:
>> > > > > >  KL>  What bothers me most are his lack of actual qualificatio=
ns,
>> > > > > >  and
>> > > > > > KL>   absolute dismissal of what he has not experienced..as if=
 he
>> > > > > > KL>   defines Linux usage for everyone.  That attitude is=20
>> > > > > > KL>   dangerous,
>> > > > > > KL>   because he is educating those outside of the accessibili=
ty
...
>> > > > > >  qualifications.
>> > > > > >    I
>> > > > > >    also know first hand that he is open to constructive feedba=
ck=20
>> > > > > >    and
>> > > > > >  I
...
>> > > > > >    opportunity
>> > > > > >  to
>> > > > > >    find out what=C3=A2?Ts possibly missing in making anybody b=
etter
>> > > > > >    informed.
>> > > > > >=20
...
>> > > > > >  what
>> > > > > >    reasonable free software alternatives to a less or more=20
>> > > > > >    standard
>> > > > > >    desktop
>> > > > > >    with Orca and a software synthesizer can you see for a comm=
on
>> > > > > >  blind
>> > > > > >    user
...
>> > > > > >  hired
>> > > > > >    to
>> > > > > >    improve accessibility.  This tells something about the stat=
e=20
>> > > > > >    of
>> > > > > >  the
>> > > > > >    matters.  We cannot expect that a single person will fix al=
l=20
>> > > > > >    the
>> > > > > >    kinds
...
>> > > > > >    works at
>> > > > > >    his
>> > > > > >    job focusing on certain areas currently seen there as urgen=
t=20
>> > > > > >    ones
>> > > > > >    and I
